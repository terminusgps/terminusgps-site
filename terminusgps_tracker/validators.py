...
def validate_does_not_contain_forbidden_symbol(value: str) -> None:
    ...
    if any(char in list(forbidden_symbols) for char in value):
        raise ValidationError(
            ...
            params={"symbols": ["less than (<)", "greater than (>)", "open curly ({)", "close curly (})", "comma (,)", "backslash (\\)"]},
            code="invalid",
        )
...
def validate_imei_number_is_available(value: str) -> None:
    ...
    with WialonSession() as session:
        ...
        if not unit_id:
            raise ValidationError(
                _("'%(value)s' was not found in the Terminus GPS database. Please ensure your IMEI # is correctly input."),
                params={"value": value.strip()},
                code="invalid",
            )

        if unit_id not in available.units:
            raise ValidationError(
...
def validate_username_is_unique(value: str) -> None:
    """Raises `ValidationError` if the value would create a non-unique user in the Terminus GPS Wialon database."""
    with WialonSession() as session:
        result = session.wialon_api.core_check_unique(**{"type": "user", "value": value}).get("result", 1)
        if result:
            raise ValidationError(
...
def validate_asset_name_is_unique(value: str) -> None:
    """Raises `ValidationError` if the value would create a non-unique avl_unit in the Terminus GPS Wialon database."""
    with WialonSession() as session:
        result = session.wialon_api.core_check_unique(**{"type": "avl_unit", "value": value}).get("result", 1)
        if result:
            raise ValidationError(
                _("'%(value)s' is taken. Please try another value."),
                params={"value": value},
...
def validate_contains_digit(value: str) -> None:
    ...
    if not any(char in string.digits for char in value):
        raise ValidationError(
            _("Ensure this value contains at least one digit."),
            code="invalid",
        )
